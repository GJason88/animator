...
public class AnimationReader {
   ...
   * <ul>
   * <li>cs3500.animator.model.Shape lines: the keyword "shape" followed by two identifiers (i.e.
   * alphabetic strings with no spaces), giving the unique name of the shape,
   * and the type of shape it is.</li>
   * <li>cs3500.animator.model.Motion lines: the keyword "motion" followed by an identifier giving
   * the name
  ...
  private static <Doc> void readShape(Scanner s, AnimationBuilder<Doc> builder) {
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException(
          "cs3500.animator.model.Shape: Expected a name, but no more input available");
    }
    if (s.hasNext()) {
      type = s.next();
    } else {
      throw new IllegalStateException(
          "cs3500.animator.model.Shape: Expected a type, but no more input available");
    }
  ...
  private static <Doc> void readMotion(Scanner s, AnimationBuilder<Doc> builder) {
    ...
    if (s.hasNext()) {
      ...
      throw new IllegalStateException(
          "cs3500.animator.model.Motion: Expected a shape name, but no more input available");
    }
    for (int i = 0; i < 16; i++) {
      vals[i] = getInt(s, "cs3500.animator.model.Motion", fieldNames[i]);
    }
